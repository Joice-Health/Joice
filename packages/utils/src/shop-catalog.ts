 ...
 * place that decides WHICH products Joice sells, under which slug and care
 * areas, with what editorial copy. Shared reference data on purpose: the shop
 * shelves, the PDPs, AND the companion's catalogue tool (epic 285) all sell
 * from this map, and chat product cards join back to it BY SLUG for image,
 * hue and the CarePortals id, which makes a slug rename a breaking change. Live CarePortals data supplies name, price
 ...
 */
import type { CareAreaSlug } from './care-areas';
