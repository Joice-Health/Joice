...
| Prod model id `us.anthropic.claude-sonnet-5` was invalid (undated) | Dated Sonnet 4.5 profile everywhere a default lived (env, tfvars default, compose, .env.example, admin presets); verify with `aws bedrock list-inference-profiles` before switching prod off Nova. **Correction, 2026-09-17 (ENG-167):** the diagnosis in this row was wrong. `us.anthropic.claude-sonnet-5` is the real US geo profile id on the AWS model card; newer Claude profiles are undated. The invoke most likely failed because the account's Anthropic use-case form was not approved, which Bedrock can surface as "model does not exist" ([08](08-troubleshooting.md)). Defaults and the admin preset now use that id |
