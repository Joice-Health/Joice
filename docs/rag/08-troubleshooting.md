...
| `ResourceNotFoundException: Model use case details have not been submitted for this account` — or the SDK surfacing it as a misleading `404 The model 'X' does not exist` | The one-time **Anthropic use-case form** was never submitted (Titan works without it; Anthropic models require it). This is exactly what blocked the first local test | Console → Bedrock → Model access → request Anthropic models → fill the use-case form; wait ~15 min, retry. Verify with `aws bedrock-runtime invoke-model` (the raw CLI gives the honest error; the SDK's 404 hides it) |
