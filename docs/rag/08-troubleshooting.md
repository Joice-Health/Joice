...
| `ValidationException: ... on-demand throughput isn't supported` (or "invocation with the given model identifier is not supported") | Using the bare model id instead of the **cross-region inference profile** | `RAG_MODEL` must carry the `us.` prefix: `us.anthropic.claude-sonnet-4-5-20250929-v1:0` |
| `ValidationException` at invoke with a plausible-looking model id (e.g. `us.anthropic.claude-sonnet-5`) | Bedrock inference-profile ids are **dated** — an undated id is invalid no matter how plausible it reads | Run `aws bedrock list-inference-profiles --region us-east-1` and copy the exact id |
