...
```bash
# Prod — chat/voice live in the brain's log group, everything else in the api's
aws logs filter-log-events --log-group-name /ecs/joice-brain \
  --filter-pattern '{ $.reqId = "PASTE-THE-ID" }'
aws logs filter-log-events --log-group-name /ecs/joice-api \
  --filter-pattern '{ $.reqId = "PASTE-THE-ID" }'

# Local
docker compose logs brain api | grep PASTE-THE-ID
```
...
|---|---|---|
| The one-shot `migrate` service fails (and api/brain never start — they wait on it) with `type "vector" does not exist` or `could not open extension control file` | Postgres is running the plain `postgres:17-alpine` image (old container/volume from before the switch) | `docker compose up -d --force-recreate postgres migrate api brain` — compose now uses `pgvector/pgvector:pg17`. The data volume carries over |
...
| Container stops seeing file edits (hot reload dead) | Stale Docker Desktop mount cache (known issue, root CLAUDE.md) | `docker compose up -d --force-recreate api brain web` |
...
| Chat endpoint returns `{"error":"Something went wrong..."}` locally | No/expired AWS creds in `.env`, or Bedrock model access not enabled on the account | **`./scripts/dev-aws-refresh.sh`** (recreates the **brain** — compose only reads `.env` at start). Verify model access in the Bedrock console |
| `429 Too Many Requests` while iterating | The 5/min/IP rate limit | Restart the **brain** container (the window is in-memory), or temporarily raise `max` in `apps/brain/src/app.ts` (don't commit) |

## Bedrock

| Symptom | Likely cause | Fix |
|---|---|---|
| `AccessDeniedException` … `is not authorized to perform: bedrock:InvokeModel` | IAM: locally your identity lacks the action; in ECS the task-role policy doesn't cover the model ARN | Local: attach Bedrock permissions to your identity. Prod: the **brain** task role policy (`infra/iam.tf → brain_bedrock`) must cover both the **inference-profile ARN** and `foundation-model/anthropic.*` in **all** regions the profile fans out to (already written as `bedrock:*` on the foundation-model line — don't "tighten" it to one region) |
...
| `ExpiredTokenException` in the **brain** logs | The static SSO creds in `.env` expired (sessions on this account last ~1 hour) | **`./scripts/dev-aws-refresh.sh`** — logs in (browser approval), rewrites the `AWS_*` lines in `.env`, recreates the **brain** container, waits for health. The api has no AWS dependencies, so this never appears there |
| `ValidationException: ... on-demand throughput isn't supported` (or "invocation with the given model identifier is not supported") | Using the bare model id instead of the **cross-region inference profile** | `RAG_MODEL` must carry the `us.` prefix: `us.anthropic.claude-sonnet-5` (Sonnet 5 has no in-region option on `bedrock-runtime` at all; only geo and global profiles) |
| `ValidationException` or "model does not exist" at invoke with a plausible-looking model id | Either the id really is wrong, or the account's Anthropic use-case form is not approved and the error is masquerading (row above). The id shape is not a reliable tell: older profiles are dated (`us.anthropic.claude-sonnet-4-5-20250929-v1:0`), newer ones are not (`us.anthropic.claude-sonnet-5`). An earlier version of this row claimed undated ids are always invalid; that was wrong, see the 2026-09-17 correction in [11](11-brain-audit.md) | Run `aws bedrock list-inference-profiles --region us-east-1` and copy the exact id, then run the Converse preflight in [runbook step 2](06-deployment-runbook.md) from an admin identity; it separates "bad id" from "account not approved" |
| Sonnet 5 answers cut off early, or a tool-mode answer fails on its second model call | `thinking` reached Bedrock enabled: adaptive thinking is Sonnet 5's default, it spends the same `maxTokens` as the answer, and the agent loop does not round-trip reasoning blocks | `toConverseInput` (`packages/brain/src/providers/bedrock.ts`) sends `thinking: {type: "disabled"}` for ids matching `anthropic.claude-sonnet-5`. A custom id that does not match that pattern (an application inference profile ARN, say) skips it; extend the pattern rather than raising the token cap |
...
| Task can't reach the database (connection timeout) | RunTask launched with the wrong security group — RDS admits only the api and **brain** task SGs | Use the paste-ready `terraform output -raw ingest_run_task_command`, which bakes in the right SG/subnets |
...
| Symptom | Likely cause | Fix |
|---|---|---|
| Every question returns the "not covered" fallback | Empty `note_chunks` (ingest never ran), or the match threshold too high for this corpus | `SELECT count(*) FROM note_chunks;` first. Then experiment with the **Match threshold** on `/admin/brain` (live, no deploy) — log the similarity scores of retrieved rows for a few known-good questions to calibrate |
| Answers cite barely-related notes | Match threshold too low, or Notes per answer (topK) too high for a small corpus | Raise the threshold / lower topK on `/admin/brain` |
...
| Answers ignore the notes / add outside knowledge | System-prompt drift after edits | The grounding rules live in the code-level safety floor (`SAFETY_FLOOR` / `TOOL_SAFETY_FLOOR` in `packages/brain/src/generation/prompt.ts`), which admin fields can't remove — re-test with an off-corpus question after any tone/instruction change |
...
| Symptom | Likely cause | Fix |
|---|---|---|
| Deploy fails at the migrate step (CI stops before updating services) | The `joice-migrate` one-shot task exited non-zero | `aws logs tail /ecs/joice-migrate --since 15m` — look at the first error. `CREATE EXTENSION` requires the master user (the `joice/database-url` secret is the master user, so this only breaks if that changed) |
| `/api/brain/chat` 403s | You're hitting the ALB directly — origin lock rejects requests without CloudFront's header (by design) | Go through `https://joicehealth.com` |
