...
| **Toolbelt (per-ability access)** | Tool mode only. Each ability is Off or has a minimum lifecycle stage (Everyone / Leads and up / Users and up / Subscribers only; the universal visitor-lead-user-subscriber ladder). Below the stage the ability is invisible to the model, never refused. Research-library search below someone's stage means the classic pipeline answers instead. See `13-toolbelt.md` |
