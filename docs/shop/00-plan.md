...
```mermaid
flowchart LR
    H["/\nlanding (was /home)"] -->|Get started| C["/shop\ncatalogue"]
    C -->|View| P["/shop/[id]\nproduct"]
...
| 2026-08-31 | The storefront landing moved from /home to the site root and joicehealth.com is live | the storefront is the public site now, so it owns the naked domain; /home 308s to / (next.config.ts) because auditors hold direct URLs. The `shop` flag outranks every other flag at the root: on, the storefront renders; off, the public falls to /waitlist and then /coming-soon per the existing chain. The storefront keeps its noindex, Shaun's call: reachable at the root, not searchable. The future main-site landing parked at team-gated /preview; SITE_LAUNCHED stays false, so every other site page keeps the team gate (sc-251) |
| 2026-09-01 | A public jurisdiction disclosure at /states, linked from every footer, the FAQ and the landing | LegitScript review Question 24 (Certification Standard #5, Patient Services) requires the live site to disclose every state, territory and country served; requested by Richard, blocking. The page is flag-free like the legal pages, exempt from the site's noindex (its page metadata overrides the (legal) layout) and listed in app/sitemap.ts, because the analyst loads it cold. Availability facts sit in one constants block at the top of the page so a licensure change is a one-line edit; the stale waitlist-era site meta description was replaced in the same push (sc-275) |
| 2026-09-01 | The storefront moved from /shop and /checkout to /store and /store/checkout (route group renamed (store)); behavior unchanged, no redirects from the old paths | the real shop (docs/shop/01-commerce.md, epic 261) owns everything under /shop team-gated; nobody outside the team had seen the site yet (Shaun's call), so forward machinery would have had no beneficiary. The /store prefix deliberately reads as an ordinary storefront section, never as a temporary site, and every cert link now goes through lib/cert-routes.ts so post-audit retirement is one grep (sc-263) |
