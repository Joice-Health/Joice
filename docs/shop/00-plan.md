...
| 2026-09-01 | The storefront moved from /shop and /checkout to /store and /store/checkout (route group renamed (store)); behavior unchanged | the real shop (docs/shop/01-commerce.md, epic 261) takes the clean routes team-gated; auditor URLs keep working (unconditional next.config 307s for the deep shapes, a cookie-aware middleware forward for exact /shop and /checkout), and the /store prefix deliberately reads as an ordinary storefront section, never as a temporary site. Every cert link now goes through lib/cert-routes.ts so post-audit retirement is one grep (sc-263) |
