...
- **AWS**: Bedrock, Transcribe and Polly, via the `joice-brain-task` role.
- **CarePortals public API** (`src/ports/careportals-catalog.ts`): the catalogue tool's
  ...
  cold-and-dark throws so the loop reports "could not check" rather than a false no. The api service has
